...
int readXbee(void){
    short ARRAY_SIZE = 5; //We're assuming that the XBee is going to send 5 shorts. Change it as you see fit.
    termios Xbee;
    int short_in[5];
    int fd, shorts_read;
    ...
    short_in[0] = read(fd, short_in, ARRAY_SIZE); /* Reads ttyO port, stores data into short_in. */

    for (int i = ARRAY_SIZE; i < ARRAY_SIZE; i++){
        cout << short_in[i] << endl;
    }
    close(fd);
    return 1;
}
